...
- Measured in Chrome with every stub filled, on Twin Yards, with nobody touching the keyboard: the enemy sat out its thirty-second head start, then bought a fourth harvester, a power plant, a barracks, two riflemen, a war factory and two tanks, marched with all but one guard, and wrecked the undefended refinery. The banner read `You lost` at 2 minutes 54 seconds. The same run reads 2 minutes 17 seconds with `HEAD_START` at 0 and 2 minutes 39 seconds at 20, so the dial buys what it says it buys.
- Measured in Chrome, two versions of the head start: one that let the enemy's harvesters dig while it slept moved its first wave by thirteen seconds out of forty-five, because it banked the credits and spent them on waking. Stopping the digging too is what makes the seconds real. Raising `THINK_EVERY` from 1 to 3 moved it by three seconds, because the commander waits on credits and not on its next think; both failures are recorded in `numbers.ts` so the dial is not retried.
- Measured in Chrome with every stub filled and `HEAD_START` at 0, on Twin Yards, played through Playwright on the enemy's own plan, sending whatever it had as soon as it had four: blue lost at 2 minutes 30 seconds, aiming at the refinery and at the enemy ore field alike. The script attacks in fours, which is the mistake the README names; no scripted line has beaten the commander since infantry arrived, and the dials that move that are `ATTACK_FORCE`, `ATTACK_EDGE` and where the two infantry lines sit in `RED_PLAN`.
- Measured in Chrome with `HEAD_START` at 0, played through Playwright as a rifleman rush — a barracks, no war factory, seven infantry: the two riflemen `RED_PLAN` keeps at home held the rush until the first enemy tank rolled out, and blue lost at 5 minutes 15 seconds. With those two lines moved below the war factory the enemy has nothing at home when the rush lands. That is the plan deciding a game, and it is the claim project 22 made about a table being the strategy.
