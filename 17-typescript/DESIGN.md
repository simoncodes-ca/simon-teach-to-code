...
> Recorded from the built page; see `17-typescript/` for the implementation.
>
> `styles.css` is copied unchanged from `16-tanks/`, and so is `tanks.html` apart from its one script tag. Project 17 changes the code only. The one new thing on screen is the type checker's error badge in the bottom right corner, which `vite-plugin-checker` draws in its own style.
